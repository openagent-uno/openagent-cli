...
# Keep the transport import lazy: commands such as ``--help`` and validation
# should not initialize Iroh or touch the user's network store.
if False:  # TYPE_CHECKING placeholder — satisfies IDEs without importing
    from .client import GatewayClient, TERMINAL_OUTPUT, TERMINAL_EXIT, TERMINAL_ERROR  # noqa: F401
...
# ── Unified history/search helpers ──────────────────────────────────────
...
_SINCE_RE = re.compile(r"^(?P<count>[1-9][0-9]*)(?P<unit>[smhdw])$")


def _since_to_utc(value: str | None, *, now: datetime | None = None) -> str | None:
    """Convert CLI durations such as ``7d`` to an inclusive UTC lower bound."""
    if value is None:
        return None
    match = _SINCE_RE.fullmatch(value.strip().lower())
    if not match:
        raise ValueError("--since must be a positive duration such as 30m, 12h, 7d or 2w")
    count = int(match.group("count"))
    seconds = {"s": 1, "m": 60, "h": 3600, "d": 86400, "w": 604800}[match.group("unit")]
    current = now or datetime.now(timezone.utc)
    if current.tzinfo is None:
        current = current.replace(tzinfo=timezone.utc)
    result = current.astimezone(timezone.utc) - timedelta(seconds=count * seconds)
    return result.isoformat().replace("+00:00", "Z")


def _request_from_time(since: str | None, from_time: str | None) -> str | None:
    if since and from_time:
        raise ValueError("--since and --from are mutually exclusive")
    return from_time or _since_to_utc(since)


def _json_output(payload: dict) -> None:
    # ``click.echo`` avoids Rich markup/ANSI in machine-readable output.
...
def _print_history_page(page: dict) -> None:
    table = Table(title="Unified history")
    ...
    for item in page.get("items") or []:
        if not isinstance(item, dict):
            continue
        table.add_row(
            _short_time(item.get("occurred_at")),
    ...
    if page.get("has_more"):
        console.print("[dim]More results are available; continue with --cursor or use --all.[/dim]")


def _snippet_text(matches: Any) -> Text:
    rendered = Text()
    if not isinstance(matches, list) or not matches:
        return rendered
    fragments = matches[0].get("fragments") if isinstance(matches[0], dict) else None
    for fragment in fragments if isinstance(fragments, list) else []:
        if not isinstance(fragment, dict):
            continue
        rendered.append(
            str(fragment.get("text") or ""),
            style="bold yellow" if fragment.get("highlight") else None,
        )
    return rendered


def _print_search_page(page: dict) -> None:
    table = Table(title="Operational search")
    table.add_column("When", style="dim", no_wrap=True)
    table.add_column("Root", style="cyan", no_wrap=True)
    table.add_column("Title")
    table.add_column("Match")
    table.add_column("Target", style="dim")
    for result in page.get("items") or []:
        if not isinstance(result, dict):
            continue
        root = result.get("root") if isinstance(result.get("root"), dict) else {}
        table.add_row(
            _short_time(root.get("occurred_at")),
            str(root.get("kind") or "unknown"),
            Text(str(root.get("title") or "Untitled")),
            _snippet_text(result.get("matches")),
            Text(_target_label(result.get("target"))),
        )
    console.print(table)
    coverage = page.get("coverage") if isinstance(page.get("coverage"), dict) else {}
    if coverage and not coverage.get("complete", True):
        console.print(
            f"[yellow]Search coverage is partial ({coverage.get('state', 'unknown')}).[/yellow]"
        )
    if page.get("has_more"):
        console.print("[dim]More results are available; continue with --cursor or use --all.[/dim]")


def _remote_error_details(exc: BaseException) -> tuple[int, str, bool, str | None]:
    if isinstance(exc, UnsupportedCapability):
        return EXIT_UNSUPPORTED, "unsupported", False, None
    if isinstance(exc, PermissionError):
        return EXIT_AUTH, "unauthorized", False, None
    if isinstance(exc, RemoteAPIError):
        if exc.is_unsupported:
            return EXIT_UNSUPPORTED, "unsupported", False, exc.request_id
        if exc.code in {"unauthorized", "forbidden"} or exc.status in {401, 403}:
            return EXIT_AUTH, exc.code, exc.retryable, exc.request_id
        if exc.code in {"warming", "degraded", "rate_limited"} or exc.status in {429, 503}:
            return EXIT_UNAVAILABLE, exc.code, exc.retryable, exc.request_id
        if exc.code in {
            "invalid_request", "cursor_stale", "request_too_large", "unprocessable_query",
        } or exc.status in {400, 409, 413, 422}:
            return EXIT_INVALID_REQUEST, exc.code, exc.retryable, exc.request_id
        return EXIT_SERVER, exc.code, exc.retryable, exc.request_id
    if isinstance(exc, (aiohttp.ClientError, asyncio.TimeoutError, ConnectionError, OSError)):
        return EXIT_NETWORK, "network_error", True, None
    if isinstance(exc, RemoteProtocolError):
        return EXIT_SERVER, "invalid_response", False, None
    return EXIT_SERVER, "internal_error", False, None


def _emit_remote_error(exc: BaseException, *, as_json: bool) -> int:
    exit_code, error_code, retryable, request_id = _remote_error_details(exc)
    error = {"code": error_code, "retryable": retryable}
    if request_id:
        error["request_id"] = request_id
    if as_json:
        # Deliberately omit the server's free-form message: it could echo the
        # private search query. Machine consumers still receive stable code,
        # retryability and request id.
        click.echo(json.dumps({"error": error}, sort_keys=True), err=True)
    else:
        messages = {
            "unsupported": "This server does not support the requested history/search capability.",
            "unauthorized": "Authentication failed for this server/account.",
            "forbidden": "This account is not authorized for the requested data.",
            "warming": "Search is still warming; retry later.",
            "degraded": "Search is temporarily degraded; history may still be available.",
            "rate_limited": "Search is rate limited; retry later.",
            "cursor_stale": "The result cursor is stale; repeat the request without --cursor.",
            "network_error": "Could not reach the OpenAgent gateway.",
            "invalid_response": "The gateway returned an invalid response.",
        }
        console.print(Text(messages.get(error_code, "The gateway rejected the request."), style="red"))
        if request_id:
            console.print(Text(f"Request id: {request_id}", style="dim"))
    return exit_code
...
def _print_help() -> None:
    ...
    rows = [
        ...
        ("/update", "Check for and install updates on the remote server"),
...
@click.group()
@click.version_option(version=__version__, prog_name="openagent-cli")
def cli():
...
async def _run_connect(
    *,
    ...
    from .network.helpers import parse_handle_at_network
    from .network import user_store
    from .network.client.login import (
        LoginError,
        register as net_register,
        login as net_login,
    )
    from .network.identity import load_or_create_identity
    from .network.iroh_node import IrohNode
    from .network.ticket import InviteTicket, TicketError, looks_like_ticket
    import getpass
    ...
    from .network.helpers import coordinator_node_id_to_pubkey_bytes
...
async def _open_gateway_for_rest(
    ...
    *,
    ...
    net = user_store.find(store, network_name, principal_handle)
    if net is None:
        if not quiet:
            target = (
                f"{principal_handle}@{network_name}" if principal_handle else network_name
            )
            console.print(f"[red]Unknown network membership: {target}[/red]")
        return None, None
...
def _finish_remote_command(exit_code: int) -> None:
    if exit_code:
        raise click.exceptions.Exit(exit_code)


async def _disconnect_remote(client) -> None:
    try:
        await client.disconnect()
    except Exception:
        # The request outcome and machine-readable exit code must not be
        # replaced by a best-effort transport teardown failure.
        pass


@cli.command("server-info")
@click.option("--network", "network_name", default=None,
              help="Which joined network to query (default: active).")
@click.option("--agent", "agent_handle", default=None,
              help="Specific agent handle on the network.")
@click.option("--handle", "principal_handle", default=None,
              help="Account handle on that network (required when ambiguous).")
@click.option("--password", default=None,
              help="Prompt only when the cached device certificate is stale.")
@click.option("--json", "as_json", is_flag=True,
              help="Emit stable machine-readable JSON.")
def server_info_cmd(network_name: str | None, agent_handle: str | None,
                    principal_handle: str | None, password: str | None, as_json: bool):
    """Show the remote server and independently versioned capabilities."""
    _finish_remote_command(asyncio.run(
        _run_server_info_cli(
            network_name, agent_handle, principal_handle, password, as_json,
        )
    ))


async def _run_server_info_cli(
    network_name: str | None,
    agent_handle: str | None,
    principal_handle: str | None,
    password: str | None,
    as_json: bool,
) -> int:
    client = None
    try:
        client, _net = await _open_gateway_for_rest(
            network_name, password, agent_handle,
            principal_handle=principal_handle, quiet=True,
        )
        if client is None:
            raise ConnectionError("no active OpenAgent network")
        capabilities = await client.operational_api.capabilities()
        payload = {
            "server": {
                "agent_name": client.agent_name,
                "agent_version": client.agent_version,
                "agent_handle": client.agent_handle,
                "network_id": client.network_id,
                "target_agent": client.target_handle,
                "principal_handle": client.principal_handle,
            },
            "capabilities": capabilities,
            "legacy": capabilities is None,
        }
        if as_json:
            _json_output(payload)
        else:
            server = payload["server"]
            table = Table(title="OpenAgent server")
            table.add_column("Field", style="cyan")
            table.add_column("Value")
            for key, value in server.items():
                table.add_row(key.replace("_", " ").title(), Text(str(value or "—")))
            if capabilities is None:
                table.add_row("Capabilities", Text("legacy / not advertised", style="yellow"))
            else:
                table.add_row("API revision", str(capabilities.get("api_revision") or "—"))
                storage = capabilities.get("storage")
                if isinstance(storage, dict):
                    table.add_row("Storage phase", str(storage.get("phase") or "—"))
                    table.add_row("Search state", str(storage.get("search_state") or "—"))
            console.print(table)
        return 0
    except Exception as exc:  # errors are classified without echoing server/query text
        return _emit_remote_error(exc, as_json=as_json)
    finally:
        if client is not None:
            await _disconnect_remote(client)


@cli.command("history")
@click.option("--type", "kinds", multiple=True, metavar="KIND[,KIND]",
              help=f"Activity kind(s): {', '.join(sorted(ACTIVITY_KINDS))}.")
@click.option("--status", "statuses", multiple=True, metavar="STATUS[,STATUS]",
              help=f"Run status(es): {', '.join(sorted(RUN_STATUSES))}.")
@click.option("--origin", default=None, help="Execution/channel origin.")
@click.option("--parent-type", default=None, metavar="KIND",
              help=f"Parent kind: {', '.join(sorted(PARENT_KINDS))}.")
@click.option("--parent-id", default=None, help="Parent id (requires --parent-type).")
@click.option("--since", default=None, metavar="DURATION",
              help="Inclusive relative lower bound, for example 7d or 12h.")
@click.option("--from", "from_time", default=None, metavar="UTC",
              help="Inclusive UTC lower bound (mutually exclusive with --since).")
@click.option("--to", "to_time", default=None, metavar="UTC",
              help="Exclusive UTC upper bound.")
@click.option("--include-children", is_flag=True,
              help="Show causal child sessions as separate rows.")
@click.option("--limit", type=click.IntRange(1, 100), default=50, show_default=True,
              help="Page size; --all still respects server snapshot quotas.")
@click.option("--cursor", default=None, help="Opaque cursor from a prior page.")
@click.option("--all", "all_pages", is_flag=True,
              help="Explicitly consume every page until next_cursor is empty.")
@click.option("--network", "network_name", default=None,
              help="Which joined network to query (default: active).")
@click.option("--agent", "agent_handle", default=None,
              help="Specific agent handle on the network.")
...
@click.option("--password", default=None,
              help="Prompt only when the cached device certificate is stale.")
@click.option("--json", "as_json", is_flag=True,
              help="Emit the canonical HistoryPage JSON object.")
def history_cmd(
    kinds: tuple[str, ...], statuses: tuple[str, ...], origin: str | None,
    parent_type: str | None, parent_id: str | None, since: str | None,
    from_time: str | None, to_time: str | None, include_children: bool,
    limit: int, cursor: str | None, all_pages: bool, network_name: str | None,
    agent_handle: str | None, principal_handle: str | None,
    password: str | None, as_json: bool,
):
    """List the authorized unified history through the remote gateway.

    Exit codes: 0 results, 1 no match, 2 invalid request, 3 auth, 4
    unsupported, 5 warming/degraded, 6 network, 7 server/protocol.
    """
    try:
        query = HistoryQuery(
            kinds=split_csv(kinds), statuses=split_csv(statuses), origin=origin,
            parent_type=parent_type, parent_id=parent_id,
            from_time=_request_from_time(since, from_time), to_time=to_time,
            include_children=include_children, limit=limit, cursor=cursor,
        )
    except ValueError as exc:
        raise click.UsageError(str(exc)) from exc
    _finish_remote_command(asyncio.run(
        _run_history_cli(
            query, all_pages, network_name, agent_handle, principal_handle,
            password, as_json,
        )
    ))


async def _run_history_cli(
    query: HistoryQuery,
    all_pages: bool,
    network_name: str | None,
    agent_handle: str | None,
    ...
    try:
        ...
        page = await api.collect_history(query, all_pages=all_pages)
        if as_json:
            _json_output(page)
        else:
            _print_history_page(page)
        return 0 if page.get("items") else EXIT_NO_MATCH
...
@click.option("--parent-type", default=None, metavar="KIND",
              help=f"Parent kind: {', '.join(sorted(PARENT_KINDS))}.")
@click.option("--parent-id", default=None, help="Parent id (requires --parent-type).")
@click.option("--root-kind", default=None, metavar="KIND",
              help=f"Restrict to a root kind: {', '.join(sorted(SEARCH_ROOT_KINDS))}.")
@click.option("--root-id", default=None, help="Root id (requires --root-kind).")
@click.option("--since", default=None, metavar="DURATION",
              help="Inclusive relative lower bound, for example 7d or 12h.")
@click.option("--from", "from_time", default=None, metavar="UTC",
              help="Inclusive UTC lower bound (mutually exclusive with --since).")
@click.option("--to", "to_time", default=None, metavar="UTC",
              help="Exclusive UTC upper bound.")
@click.option("--sort", "sort_order", type=click.Choice(sorted(SEARCH_SORTS)),
              default="relevance", show_default=True)
@click.option("--grouping", type=click.Choice(sorted(SEARCH_GROUPINGS)),
...
@click.option("--password", default=None,
              help="Prompt only when the cached device certificate is stale.")
@click.option("--json", "as_json", is_flag=True,
              help="Emit the canonical SearchPage JSON object with typed targets.")
def search_cmd(
    query_text: str, scopes: tuple[str, ...], statuses: tuple[str, ...],
    origin: str | None, parent_type: str | None, parent_id: str | None,
    root_kind: str | None, root_id: str | None, since: str | None,
    from_time: str | None, to_time: str | None, sort_order: str, grouping: str,
    limit: int, cursor: str | None, all_pages: bool, network_name: str | None,
    agent_handle: str | None, principal_handle: str | None,
    password: str | None, as_json: bool,
):
    ...
    try:
        query = SearchQuery(
            query=query_text, scopes=split_csv(scopes), statuses=split_csv(statuses),
            origin=origin, parent_type=parent_type, parent_id=parent_id,
            root_kind=root_kind, root_id=root_id,
            from_time=_request_from_time(since, from_time), to_time=to_time,
            sort=sort_order, grouping=grouping, limit=limit, cursor=cursor,
        )
    except ValueError as exc:
        raise click.UsageError(str(exc)) from exc
    _finish_remote_command(asyncio.run(
...
async def _run_search_cli(
    ...
    try:
        ...
        page = await api.collect_search(query, all_pages=all_pages)
        if as_json:
            _json_output(page)
        else:
            _print_search_page(page)
        return 0 if page.get("items") else EXIT_NO_MATCH
    except Exception as exc:
        return _emit_remote_error(exc, as_json=as_json)
    finally:
...
async def _run_terminal(
    ...
    import shutil
    import uuid
    from .client import TERMINAL_OUTPUT, TERMINAL_EXIT, TERMINAL_ERROR  # lazy import
