#!/usr/bin/env python3
"""Verify the exact immutable CLI release set locally and on GitHub."""
...
parser.add_argument("--expect-draft", type=boolean)
parser.add_argument("--expect-prerelease", type=boolean)
args = parser.parse_args()

base = f"openagent-cli-{args.version}"
expected = {
...
local = {path.name: {"size": path.stat().st_size, "digest": sha256(path)} for path in files}
for sidecar_name in sorted(name for name in expected if name.endswith(".sha256")):
...
print(f"verified {len(expected)} immutable CLI release assets for {args.version}")
